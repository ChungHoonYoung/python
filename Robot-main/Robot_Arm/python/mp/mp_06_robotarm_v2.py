...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            if angle1 <= 45 and not arm_down:
                counter1 += 1
                arm_down = True
            if angle1 > 45:
                arm_down = False
                
            if angle2 <= 45 and not arm_down:
                counter2 += 1
                arm_down = True
            if angle2 > 45:
                arm_down = False
                
            if angle3 <= 45 and not arm_down:
                counter3 += 1
                arm_down = True
            if angle3 > 45:
                arm_down = False
            
            cv2.putText(image, f'Count1: {counter1}', (10, 50), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.putText(image, f'Count2: {counter2}', (10, 90), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
            cv2.putText(image, f'Count3: {counter3}', (10, 130), 
