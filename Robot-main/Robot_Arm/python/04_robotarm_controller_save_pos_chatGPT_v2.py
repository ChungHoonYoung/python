...
class RobotArmControl:
    ...
    def setup_position_combobox(self):
        ...
        # 라벨 추가
        self.label = Label(position_frame, text="전송정보 라벨", bg="black", fg="white")
        self.label.pack()

        # 시리얼 데이터를 읽어서 콤보박스에 추가하는 역할을 하는 메서드 호출
        self.master.after(100, self.read_serial_data)
    ...
    def read_serial_data(self):
        if hasattr(self, 'ser') and self.ser.is_open:
            while self.ser.in_waiting:
                ...
                if serial_data:
                    # 시리얼 데이터를 파싱하여 콤보박스에 추가
                    self.add_to_combobox(serial_data)

    def add_to_combobox(self, data):
        # 받은 시리얼 데이터를 콤보박스에 추가
        self.position_combobox['values'] = data.split(",")[:-1]  # 마지막 'd' 문자열은 제외
